<template>
    <div>
        <div class="mt-3">
            ...
            <span>/</span>
            <label active>{{$t('Kitchens')}}</label>
        </div>
        <h4 class="mt-3">{{$t('Kitchens')}}</h4>
...
export default {
    data() {
        return {
            ...
            items: [],
            fields: [
                { key: 'cooking_date', label: this.$t('cooking_date'), sortable: true, sortDirection: 'desc' },
    ...
    methods: {
        ...
        deleteRow(idVal){
            this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                ...
                this.$store.dispatch("kitchens/removeData", idVal)
                    .then(_ => {
        ...
        initData(filters) {
            this.$store
                .dispatch("kitchens/getData", filters)
                .then(res => {
        ...
        addNewItem:function(){
            this.$router.push({name: 'kitchensAddEdit'})
        },
        detailsRow(id) {
            this.$router.push({name: 'kitchensAddEdit', params: {id: id, page: 'details'}})
        },
        editRow(id) {
            this.$router.push({name: 'kitchensAddEdit', params: {id: id, page: 'edit'}})
        },
        updateData(){
            this.initData(null);
        },
        updateStatus(data) {
            ...
            this.$store
                .dispatch("kitchens/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

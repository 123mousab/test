<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Packages')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Packages')}}</h4>
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    { key: 'cost', label: 'التكلفة', sortable: true, sortDirection: 'desc' },
                    { key: 'price', label: 'السعر', sortable: true, sortDirection: 'desc' },
                    { key: 'number_of_days', label: 'عدد ايام الباقة', sortable: true, sortDirection: 'desc' },
                    { key: 'number_of_meals', label: 'عدد الوجبات للباقة', sortable: true, sortDirection: 'desc' },
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("packages/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("packages/getData", filters)
                .then(res => {
            ...
            addNewItem:function(){
               this.$router.push({name: 'packagesAddEdit'})
            },
            detailsRow(id) {
                this.$router.push({name: 'packagesAddEdit', params: {id: id, page: 'details'}})
            },
            editRow(id) {
                this.$router.push({name: 'packagesAddEdit', params: {id: id, page: 'edit'}})
            },
            updateData(){
                this.initData(null);
            },
            ...
            this.$store
                .dispatch("packages/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

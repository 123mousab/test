<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Cuisines')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Cuisines')}}</h4>
        ...
        methods: {
            changeImage(event, id) {
                if (event.target.files.length) {
                    ...
                    this.$store
                        .dispatch("cuisines/updateImage", toSend)
                        .then((_) => {
                            this.$notify.success({
                                duration: 3000,
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("cuisines/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("cuisines/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("cuisines/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

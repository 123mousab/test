<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Branches')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Branches')}}</h4>
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    { key: 'city_name', label: 'المدينة', sortable: true, sortDirection: 'desc' },
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("branches/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("branches/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("branches/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

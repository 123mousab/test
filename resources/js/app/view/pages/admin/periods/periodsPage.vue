<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('فترات العمل')}}</label>
    </div>
        <h4 class="mt-3">{{$t('فترات العمل')}}</h4>
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    { key: 'start_date', label: "بداية الوقت" },
                    { key: 'end_date', label: "انتهاء الوقت"},
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("periods/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("periods/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("periods/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

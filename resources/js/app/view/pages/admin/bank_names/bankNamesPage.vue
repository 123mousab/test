<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('البنوك')}}</label>
    </div>
        <h4 class="mt-3">{{$t('البنوك')}}</h4>
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("bank_names/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("bank_names/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("bank_names/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

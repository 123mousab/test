<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Cities')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Cities')}}</h4>
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    { key: 'country_name', label: 'الدولة', sortable: true, sortDirection: 'desc' },
                    { key: 'delegate_name', label: 'المندوب', sortable: true },
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("cities/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("cities/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("cities/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Nutrition Facts')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Nutrition Facts')}}</h4>
        ...
        methods: {
            changeImage(event, id) {
                if (event.target.files.length) {
                    ...
                    this.$store
                        .dispatch("nutrition_facts/updateImage", toSend)
                        .then((_) => {
                            this.$notify.success({
                                duration: 3000,
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("nutrition_facts/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("nutrition_facts/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("nutrition_facts/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

<template>
    <div>
        <div class="mt-3">
            ...
            <span>/</span>
            <label active>{{$t('Menus')}}</label>
        </div>
        <h4 class="mt-3">{{$t('Menus')}}</h4>
    ...
    methods: {
        ...
        deleteRow(idVal){
            this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                ...
                this.$store.dispatch("new_menus/removeData", idVal)
                    .then(_ => {
        ...
        initData(filters) {
            this.$store
                .dispatch("new_menus/getData", filters)
                .then(res => {
        ...
        addNewItem:function(){
            this.$router.push({name: 'menusAddEdit'})
        },
        detailsRow(id) {
            this.$router.push({name: 'menusAddEdit', params: {id: id, page: 'details'}})
        },
        editRow(id) {
            this.$router.push({name: 'menusAddEdit', params: {id: id, page: 'edit'}})
        },
        updateData(){
            this.initData(null);
        },
        updateStatus(data) {
            ...
            this.$store
                .dispatch("new_menus/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,

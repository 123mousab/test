<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Divisions')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Divisions')}}</h4>
        ...
        methods: {
            changeImage(event, id) {
                if (event.target.files.length) {
                    ...
                    this.$store
                        .dispatch("divisions/updateImage", toSend)
                        .then((_) => {
                            this.$notify.success({
                                duration: 3000,
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("divisions/removeData", idVal)
                    .then(_ => {
            ...
            initData(filters) {
                this.$store
                .dispatch("divisions/getData", filters)
                .then(res => {
            ...
            this.$store
                .dispatch("divisions/updateStatus", JSON.stringify(sendData))
                .then(() => {
                    this.$notify.success({
                        duration: 3000,
